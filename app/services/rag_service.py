...
def retrieve_context(question: str, symbol: str | None = None, top_k: int = 5) -> List[Dict]:
    try:
        ...
        # Render-safe: if nothing is indexed yet, just return no RAG context
        try:
        ...
        if symbol:
            symbol_res = _run_query(
                where={"symbol": symbol.upper().strip()},
                n_results=top_k,
            )
        ...
        print("FirePulse RAG retrieve_context error:", exc)
        return []